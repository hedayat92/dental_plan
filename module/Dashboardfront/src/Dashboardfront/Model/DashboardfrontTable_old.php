...
class DashboardfrontTable
{
  ...
  public function doctorDetails(){
   ...
   public function listplanservice(){
    ...
    public function saveDoctor($doc_id,$doc_firstname,$doc_lastname,$doc_email,$doc_phone,$doc_zip,$doc_address,$doc_phone2,$doc_sex,$doc_speciality,$doc_license_no,$doc_contact_person,$doc_status,$doc_username,$password){
        if($doc_status=='on') {
         $docstatus = 1;
          } else {
            $docstatus = 0;
   ...
   {
    $update_qry = "update doctor_details set cust_id='$custid',subs_id='$subsid' where doc_id=$doc_id";
    $result = $this->adapter->query($update_qry, Adapter::QUERY_MODE_EXECUTE);
    return $result;
   }
  ...
  public function patientActiveDetails($docUserID){
    ...
    {      
     ...
     public function insertPatient($plan_id,$plan_cycle,$patient_salutation,$patient_firstname,$patient_lastname,$patient_dob,$patient_ssn,$patient_sex,$patient_address,$patient_zip,$patient_phone,$patient_mobile,$patient_email,$patient_password,$pat_id,$addon,$docid,$parent_id,$docid,$patient_status,$patient_consent) 
     {
     $newDateOB = date("Y-m-d", strtotime($patient_dob));
     if($patient_password==''){
      $patient_password =0;
     }
  ...
  if($patient_consent=='on')
   ...
   if(empty($pat_id)){
     $patientinsert ="insert into patients(patient_salutation,patient_firstname,patient_lastname,patient_ssn,patient_dob,patient_sex,patient_address,patient_zip,patient_phone,patient_mobile,patient_email,patient_pass,patient_family_info,patient_lastprocedure_info,patient_consent,plan_id, doc_id,plan_start_date,plan_cycle,parent_id) values('$patient_salutation','$patient_firstname','$patient_lastname','$patient_ssn','$newDateOB','$patient_sex','$patient_address','$patient_zip','$patient_phone','$patient_mobile','$patient_email',md5($patient_password),'$patient_family_info','$patient_lastprocedure_info','$pconsent','$plan_id','$docid',now(),'$plan_cycle','$parent_id')";
       ...
       $patientinsert ="update patients set patient_salutation='$patient_salutation', patient_firstname='$patient_firstname',patient_lastname='$patient_lastname',patient_ssn='$patient_ssn',patient_dob='$newDateOB',patient_address='$patient_address',patient_zip='$patient_zip',patient_phone='$patient_phone' ,patient_mobile='$patient_mobile',patient_family_info='$patient_family_info',patient_lastprocedure_info='$patient_lastprocedure_info',doc_id='$docid',patient_status='$pstatus',patient_consent='$pconsent' where patient_id=".$pat_id; 
     } 
